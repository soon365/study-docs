...
​	void joio（long millis）		—表示等待参数指定的毫秒数
...
​	当多个线程同时访问同一种共享资源时，可能会造成数据的覆盖等不一致性问题，此时就需要进行线程之间的通信和协调，该机制就叫做线程的同步机制.
...
​			直接使用synchronizde关键字来修饰整个方法即可该方式等价于：

​			synchronized（this）{整个方法体的代码}

#### 1.3原理分析

​	当多个线程调用start方法后同时去抢占共享资源，由于同步锁的存在导致只有个线当该程能够抢到共享资源并进行加锁处理，其它没有抢到共享资源的线程进入阻塞状态，当该线程执行完毕所有锁定的代码后自动释放同步锁，此时阻塞状态的所有线程继续抢占共享资源，抢不到的线程再次回到阻塞状态。

#### 1.4实现的注意方式

- 使用synchronized保证线程同步应当注意：

  1.多个需要同步的线程在访问该同步块时，看到的应该是一个所对象引用。

  2.在使用同步块应当尽量减少同步范围以提高并发的执行效率。
...
​	当发送数据时，需要对发送的内容按照上述七层模型进行层层加包在发送出去；
...
​	IP地址本质上是由32位二进制组成的整数，叫做IPv4，当然也有128位二进制组成的整数，叫做IPv6。
...
​	端口号本质生是由16位二进制组成的整数，标识的范围：0~65535,其中0~1024之间的端口号通常被系统占用，因此开发中从1025开始使用
...
#### 3.2 ServerSocket类的概述

- java.net.ServerSocket类主要用于描述服务类套接字信息。
...
- java.net.Socket类主要用于秒速客户端套接字，是两台机器间通信的断点。
...
- 客户端 Socket与服务器端 Socket对应,都包含输入,输出流
- 客户端的socket.getlnputStream()连接于服务器socket.getOutputStream()
...
​	需求分析文档 => 概要设计文档 => 详细设计文档 => 编码和测试 => 维护和升级
...
- 成复用原则(Composite Reuse Principle)
